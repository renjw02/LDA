...
def update_params_for_one_item(K, n_word_types, word_indices, counts, gammas, phi_d, log_betas, alpha, max_iter_d,
    ...
    while i < max_iter_d and delta >= tol_d:
        ...
        while n < n_word_types:
            ...
            ## Your codes here.
            for k in range(K):
                new_phi_dn[k] = np.exp(psi_gammas[k] + log_betas[w][k])
            new_phi_dn_sum = np.sum(new_phi_dn)
            phi_d[n] = new_phi_dn / new_phi_dn_sum

            n += 1

        gammas = alpha + np.sum((phi_d.T * counts).T, axis=0)
        psi_gammas = psi(gammas)
...
def compute_bound_for_one_item(K, n_word_types, word_indices, count_vector, alpha, gammas, psi_gammas, phi_d,
    ...
    ## Your codes here.
    delta_psi_gammas = psi_gammas - psi(np.sum(gammas))

    bound1 = gammaln(K * alpha) - K * gammaln(alpha) + np.sum((alpha - 1) * delta_psi_gammas) + np.sum(
        (phi_d * delta_psi_gammas).T * count_vector)

    bound2 = 0.0
    for n in range(n_word_types):
        for k in range(K):
            bound2 += phi_d[n][k] * log_betas[word_indices[n], k] * count_vector[n]

    bound3 = -gammaln(np.sum(gammas)) + np.sum(gammaln(gammas)) - np.sum((gammas - 1) * delta_psi_gammas) 
    - np.sum((phi_d * np.log(phi_d)).T * count_vector)
